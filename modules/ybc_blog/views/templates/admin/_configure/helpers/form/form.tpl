    ...
    {else}
        ...
        {if $input.type == 'products_search'}
            <div class="col-lg-9">
                ...
                <p class="help-block">{l s='Related products are displayed on post details page' mod='ybc_blog'}</p>
            </div>
        {/if}
    {/if}
{/block}
	...
	{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
		<div class="panel-footer">
            {if isset($cancel_url) && $cancel_url}
                <a class="btn btn-default" href="{$cancel_url|escape:'html':'UTF-8'}"><i class="process-icon-cancel"></i>{l s='Back' mod='ybc_blog'}</a>
            {/if}
