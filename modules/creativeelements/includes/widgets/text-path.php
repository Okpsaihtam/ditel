...
class WidgetTextPath extends WidgetBase
{
    ...
    protected function render()
    {
        ...
        ?>
        <svg <?= $this->getRenderAttributeString('svg') ?>>
            <path <?= $this->getRenderAttributeString('path') ?>/>
            <text>
                <textPath href="#<?= $path_id ?>" startOffset="<?= $start_offset ?>%">
                    <?= $has_link ? "<a {$this->getRenderAttributeString('link')}>$text</a>" : $text ?>
                </textPath>
            </text>
        </svg>
        <?php
    ...
    protected function _contentTemplate()
    {
        ?>
        <#
        var SVG_PATHS = <?= json_encode(self::SVG_PATHS) ?>,
            pathId = 'e-path-' + view.getID(),
            isRtl = settings.text_path_direction ? 'rtl' === settings.text_path_direction : <?= (int) is_rtl() ?>,
