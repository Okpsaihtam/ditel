...
class WidgetProductCarousel extends WidgetProductBase
{
    ...
    {
        return ['shop', 'store', 'product', 'carousel', 'slider'];
    }
    ...
    protected function render()
    {
        ...
        if (is_admin() && ('custom' !== $settings['skin'] || 'prices-drop' === $settings['listing'])) {
            return print '<div class="ce-remote-render"></div>';
        }
        if (empty($this->context->currency->id)) {
            return;
        }
        if ($settings['randomize'] && in_array($settings['listing'], ['category', 'viewed', 'products', 'related'])) {
        ...
        $products = $this->getProducts(
            ...
            $settings['products'],
            $settings['related_product_id'],
            $settings['manufacturer_id']
        );
