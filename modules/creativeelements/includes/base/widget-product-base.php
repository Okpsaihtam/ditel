...
abstract class WidgetProductBase extends WidgetBase
{
    ...
    public function __construct($data = [], $args = [])
    {
        ...
        $this->parentTheme = $this->context->shop->theme->get('parent');
    ...
    protected function getCategoryOptions()
    {
        ...
        if (is_admin()) {
            $categories[0] = __('Products in the same category');
    ...
    protected function getManufacturerOptions()
    {
        ...
        if (is_admin()) {
            $manufacturers[0] = __('Products with the same brand');

            foreach (\Manufacturer::getLiteManufacturersList($this->context->language->id) as &$manufacturer) {
                $manufacturers[$manufacturer['id']] = "#{$manufacturer['id']} {$manufacturer['name']}";
            }
        }
    ...
    protected function getProducts($listing, $order_by, $order_dir, $limit, $id_category = 2, $products = [], $related_product_id = 0, $manufacturer_id = 0)
    {
        $tpls = [];
        ...
        if ('viewed' === $listing) {
            ...
            foreach ($products as &$id_product) {
                $tpls[] = $this->getProduct($id_product);
            }
            return $tpls;
        }
        if ('products' === $listing) {
        ...
        switch ($listing) {
            ...
            case 'related':
                if ($related_product_id) {
                    $products = \Product::getAccessoriesLight($this->context->language->id, (int) $related_product_id);
                } elseif ($this->context->controller instanceof \ProductController) {
                    $products = \Product::getAccessoriesLight($this->context->language->id, $this->context->controller->getProduct()->id);
                    ...
                    while ($i--) {
                        $related_products = \Product::getAccessoriesLight($this->context->language->id, $cart['products'][$i]->id);
                ...
                break;
            case 'manufacturer':
                if(!$manufacturer_id && $this->context->controller instanceof \ProductController) {
                    $manufacturer_id = $this->context->controller->getProduct()->id_manufacturer;
                }
                $manufacturer  = new \Manufacturer((int) $manufacturer_id);
                $searchProvider = new \PrestaShop\PrestaShop\Adapter\Manufacturer\ManufacturerProductSearchProvider($translator, $manufacturer);
                $query->setSortOrder(new \PrestaShop\PrestaShop\Core\Product\Search\SortOrder('product', $order_by, $order_dir));
                break;
        }

        if ('category' === $listing && !$id_category && $this->context->controller instanceof \CartController) {
            $cart = $this->context->controller->cart_presenter->present($this->context->cart, true);
    ...
    protected function fetchMiniature(array &$settings, $product)
    {
        ...
        if ($on_sale && in_array('sale', $settings['show_badges'])) {
            $badges['sale'] = $settings['badge_sale_text'] ?: $product[
        ...
        if (!empty($product['flags']['new']['label']) && in_array('new', $settings['show_badges'])) {
            $badges['new'] = $settings['badge_new_text'] ?: $product['flags']['new']['label'];
        }
        if (!empty($product['flags']['pack']['label']) && in_array('pack', $settings['show_badges'])) {
            $badges['pack'] = $product['flags']['pack']['label'];
        }
        if (!empty($product['flags']['out_of_stock']['label']) && in_array('out', $settings['show_badges'])) {
        	$badges['out'] = $settings['badge_out_text'] ?: $product['flags']['out_of_stock']['label'];
    	}
        ...
        <article class="elementor-product-miniature" <?= $this->getRenderAttributeString($article) ?>>
            ...
            <?php if ($show_atc) : ?>
                <form class="elementor-atc" action="<?= esc_attr($atc_url) ?>">
                    <input type="hidden" name="qty" value="<?= max(1, (int) $min_qty) ?>">
                    <button type="submit" class="elementor-button elementor-size-<?= $settings['atc_size'] ?>" data-button-action="add-to-cart"
                        <?= !empty($product['flags']['out_of_stock']['label']) ? 'disabled="disabled"' : '' ?>>
                        <span class="elementor-button-content-wrapper">
