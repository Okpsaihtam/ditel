...
abstract class ControlsStack extends BaseObject
{
    ...
    public function addControl($id, array $args, $options = [])
    {
        ...
        if (empty($args['type']) || $args['type'] !== ControlsManager::SECTION) {
            ...
            if (null !== $target_section_args) {
                ...
                die(sprintf('%s::%s: Cannot add a control outside of a section (use `startControlsSection`).', get_called_class(), __FUNCTION__));
            }
        }
    ...
    {
        if ($this->injection_point) {
            die('A controls injection is already opened. Please close current injection before starting a new one (use `endInjection`).');
        }
