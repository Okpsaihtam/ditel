...
class Helper
{
    ...
    {
        $wrapfix = [
            'classic' => version_compare(_PS_VERSION_, '1.7.6', '<') ? 'featured-products wrapfix' : 'products',
    ...
    public static function getProductImageLink($image)
    {
        ...
        $ext = \Tools::substr($url, strrpos($url, '.') + 1);
...
function wp_enqueue_scripts()
{
    if (is_admin()) {
        ...
        wp_register_script('iris', _CE_ASSETS_URL_ . 'lib/iris/iris.min.js', ['jquery-ui-draggable', 'jquery-ui-slider'], '1.1.1', true);
...
function parse_shortcode($match)
{
    ...
    if ('$' === $match[2][0]) {
        ...
        if (is_array($result) || $result instanceof \PrestaShop\PrestaShop\Adapter\Presenter\AbstractLazyArray) {
            $array = [];
            foreach ($result as $key => $value) {
                $array[$key] = is_array($value) ? 'Array(…)' : $value;
            }
            $result = '<pre>' . print_r($array, true) . '</pre>';
        } elseif ('nofilter' !== $modifier) {
