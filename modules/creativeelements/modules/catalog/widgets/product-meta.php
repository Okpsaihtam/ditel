        ...
        foreach ($this->getSettings('meta_list') as $meta) {
            ...
            switch ($meta['type']) {
                ...
                case 'condition':
                    ...
                    $value = sprintf(
                        '<link itemprop="itemCondition" href="%s">',
                        esc_attr($product['condition']['schema_url'])
                    ) . esc_html($product['condition']['label']);
                    break;
                case 'reference':
                    $label = $label ?: $t->trans('Reference', [], 'Shop.Theme.Catalog');
                    $attrs = 'itemprop="sku"';
                    $value = esc_html($product['reference']);
                    break;
                default:
                    // isbn, ean13, upc, npm
                    if (empty($product[$meta['type']])) {
                        continue 2;
                    }
                    $label = \Tools::strtoupper($meta['type']);
                    $value = esc_html($product[$meta['type']]);
                    break;
            }
            printf(
                '<span class="ce-product-meta__detail ce-product-meta__%s">' .
                '   <span class="ce-product-meta__label">%s</span>' .
                '   <span class="ce-product-meta__value" %s>%s</span>' .
                '</span>',
                esc_attr($meta['type']),
                esc_html($label),
                $attrs,
