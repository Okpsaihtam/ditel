    ...
    protected function _registerControls()
    {
        ...
        $this->addControl(
            ...
            [
                ...
                'return_value' => 'show',
                'default' => 'show',
                'prefix_class' => 'ce-product-price-discount-',
            ]
        );
        ...
        $this->addGroupControl(
            ...
            [
                ...
                'selector' => '{{WRAPPER}} .ce-has-discount .ce-product-price-current',
            ]
        );
        ...
        $this->addControl(
            ...
            [
                ...
                'type' => ControlsManager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .ce-has-discount .ce-product-price-current' => 'color: {{VALUE}};',
                ],
            ]
        );
    ...
    protected function render()
    {
        ...
        <?php endif ?>
            <div class="ce-product-price <?= $product['has_discount'] ? 'ce-has-discount' : '' ?>" itemprop="offers" itemscope itemtype="https://schema.org/Offer">
                <link itemprop="availability" href="<?= esc_attr($product['seo_availability']) ?>">
                <meta itemprop="priceCurrency" content="<?= $currency['iso_code'] ?>">
                <div class="ce-product-price-current">
                    <span itemprop="price" content="<?= esc_attr($product['rounded_display_price'] ?: $product['price_amount']) ?>">
                        <?= $product['price'] ?>
                    </span>
                <?php if ($product['has_discount']) : ?>
        ...
        <?php if ($displayUnitPrice) : ?>
            <div class="ce-product-price-unit">
                <?= $t->trans('(%unit_price%)', ['%unit_price%' => $product['unit_price_full']], 'Shop.Theme.Catalog') ?>
            </div>
        <?php endif ?>

        <?php if (2 == $priceDisplay) : ?>
            <div class="ce-product-price-without-taxes">
                <?= $t->trans('%price% tax excl.', ['%price%' => $product['price_tax_exc']], 'Shop.Theme.Catalog') ?>
            </div>
        <?php endif ?>
        <?php if ($displayPackPrice) : ?>
            <div class="ce-product-price-pack">
                <?= $t->trans('Instead of %price%', ['%price%' => $noPackPrice], 'Shop.Theme.Catalog') ?>
            </div>
        <?php endif ?>
        <?php if ($product['ecotax']['amount'] > 0) : ?>
            <div class="ce-product-price-ecotax">
                <?= $t->trans('Including %amount% for ecotax', ['%amount%' => $product['ecotax']['value']], 'Shop.Theme.Catalog') ?>
            <?php if ($product['has_discount']) : ?>
                <?= $t->trans('(not impacted by the discount)', [], 'Shop.Theme.Catalog') ?>
            <?php endif ?>
            </div>
        <?php endif ?>
            <?= \Hook::exec('displayProductPriceBlock', ['product' => $product, 'type' => 'weight', 'hook_origin' => 'product_sheet']) ?>
            <?php ob_start() ?>
            <?php if (!$configuration['taxes_enabled']) : ?>
                <?= $t->trans('No tax', [], 'Shop.Theme.Catalog') ?>
            <?php elseif ($configuration['display_taxes_label']) : ?>
                <?= $product['labels']['tax_long'] ?>
            <?php endif ?>
                <?= \Hook::exec('displayProductPriceBlock', ['product' => $product, 'type' => 'price']) ?>
                <?= \Hook::exec('displayProductPriceBlock', ['product' => $product, 'type' => 'after_price']) ?>
            <?php if ($tax_shipping = trim(ob_get_clean())) : ?>
                <div class="ce-tax-shipping-delivery-label"><?= $tax_shipping ?></div>
            <?php endif ?>
        </div>
        <?php
    }
