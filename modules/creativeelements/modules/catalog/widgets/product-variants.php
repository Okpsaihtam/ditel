...
class WidgetProductVariants extends WidgetBase
{
    ...
    protected function _registerControls()
    {
        ...
        if (is_admin()) {
            ...
            $this->addControl(
                'configure',
                [
                    'raw' => __('Product Attributes') . '<br><br>' .
    ...
    protected function render()
    {
        ...
        $form = 'add-to-cart-or-refresh-' . (int) UId::$_ID->id;
        ?>
        ...
        <?php foreach ($groups as $id_attr_group => $group) : ?>
            <?php if (!empty($group['attributes'])) : ?>
                <div class="ce-product-variants__item">
                    <span class="ce-product-variants__label"><?= esc_html($group['name']) ?></span>
                <?php if ('select' === $group['group_type']) : ?>
                    <div class="ce-product-variants__select">
                        <select class="elementor-field elementor-field-textual elementor-size-<?= esc_attr($this->getSettings('select_size')) ?>"
                            form="<?= $form ?>" name="group[<?= (int) $id_attr_group ?>]" data-product-attribute="<?= (int) $id_attr_group ?>">
                        <?php foreach ($group['attributes'] as $id_attr => $group_attr) : ?>
                            <option <?= $group_attr['selected'] ? 'selected' : '' ?> value="<?= (int) $id_attr ?>"><?= esc_html($group_attr['name']) ?></option>
                        <?php endforeach ?>
                        </select>
                    </div>
                <?php elseif ('radio' === $group['group_type']) : ?>
                    ...
                    <?php foreach ($group['attributes'] as $id_attr => $group_attr) : ?>
                        <label class="ce-product-variants__radio-wrapper" aria-label="<?= esc_attr($group_attr['name']) ?>">
                            <input class="ce-product-variants__radio" form="<?= $form ?>" type="radio" <?= $group_attr['selected'] ? 'checked' : '' ?>
                                name="group[<?= (int) $id_attr_group ?>]" value="<?= (int) $id_attr ?>" data-product-attribute="<?= (int) $id_attr_group ?>">
                            <span class="ce-product-variants__option"><?= esc_html($group_attr['name']) ?></span>
                        </label>
                    <?php endforeach ?>
                    </div>
                <?php elseif ('color' === $group['group_type']) : ?>
                    ...
                    <?php foreach ($group['attributes'] as $id_attr => $group_attr) : ?>
                        <label class="ce-product-variants__radio-wrapper" title="<?= esc_attr($group_attr['name']) ?>">
                            <input class="ce-product-variants__radio" form="<?= $form ?>" type="radio" <?= $group_attr['selected'] ? 'checked' : '' ?>
                                name="group[<?= (int) $id_attr_group ?>]" value="<?= (int) $id_attr ?>" data-product-attribute="<?= (int) $id_attr_group ?>">
                        <?php if ($group_attr['html_color_code']) : ?>
                            <span class="ce-product-variants__pattern ce-product-variants__color" style="background-color: <?= esc_attr($group_attr['html_color_code']) ?>"></span>
                        <?php elseif ($group_attr['texture']) : ?>
                            <span class="ce-product-variants__pattern ce-product-variants__texture" style="background-image: url(<?= esc_attr($group_attr['texture']) ?>)"></span>
                        <?php endif ?>
                        </label>
                    <?php endforeach ?>
                    </div>
