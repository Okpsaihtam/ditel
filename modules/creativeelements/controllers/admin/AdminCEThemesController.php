                ...
                if ($tab1['class_name'] == 'AdminParentCEContent') {
                    foreach ($tab1['sub_tabs'] as &$tab2) {
                        if ($tab2['class_name'] == 'AdminCEThemes') {
                            ...
                            $tab['current'] = !$new && 'header' === $type || $this->object && 'header' === $this->object->type;
                            $tab['href'] = "$href&type=header";
                            $sub_tabs[] = $tab;

                            $tab['name'] = $this->l('Footer');
                            $tab['current'] = !$new && 'footer' === $type || $this->object && 'footer' === $this->object->type;
                            $tab['href'] = "$href&type=footer";
                            $sub_tabs[] = $tab;

                            $tab['name'] = $this->l('Page');
                            $tab['current'] = !$new && stripos($type, 'page') === 0 || $this->object && stripos($this->object->type, 'page') === 0;
                            $tab['href'] = "$href&type=page";
                            $sub_tabs[] = $tab;

                            $tab['name'] = $this->l('Product');
                            $tab['current'] = !$new && stripos($type, 'prod') === 0 || $this->object && stripos($this->object->type, 'prod') === 0;
