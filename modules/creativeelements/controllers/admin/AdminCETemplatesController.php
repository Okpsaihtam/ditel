                ...
                if ('AdminParentCEContent' === $tab1['class_name']) {
                    foreach ($tab1['sub_tabs'] as &$tab2) {
                        if ('AdminCETemplates' === $tab2['class_name']) {
                            ...
                            $tab['current'] = !$new && 'page' === $type || $this->object && !in_array($this->object->type, ['section', 'header', 'footer']);
                            $tab['href'] = "$href&type=page";
                            $sub_tabs[] = $tab;

                            $tab['name'] = $this->l('Section');
                            $tab['current'] = !$new && 'section' === $type || $this->object && in_array($this->object->type, ['section', 'header', 'footer']);
