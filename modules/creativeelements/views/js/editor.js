...
ControlSelect2ItemView = ControlBaseDataView.extend({
	...
	onReady: function onReady() {
		...
		if ('preview_settings' === elementor.panel.currentView.currentPageView.activeSection ||
			'popup_layout' === elementor.panel.currentView.currentPageView.activeSection
		) {
			var val = elementor.panel.currentView.currentPageView.model.attributes[this.model.attributes.name];
		} else {
			var val = elementor.panel.currentView.currentPageView.model.attributes.settings.attributes[this.model.attributes.name];
		}
...
var _class = function (_Marionette$Composite) {
	...
	_createClass(_class, [{
		...
		value: function exitTitleEditing() {
			...
			elementor.channels.editor.trigger('enter:' + settingsModel.get('elType') + ':_title', settingsModel);
...
helpers = {
  ...
  */
	...
	wpColorPicker: function wpColorPicker($element, options) {
		var self = this,
		    items = elementor.schemes.getScheme('color-picker').items,
		    defaultOptions = {
				width: window.innerWidth >= 1440 ? 271 : 251,
				palettes: _.pluck([
					items[7], items[8], items[1], items[5], items[2], items[3], items[6], items[4]
				], 'value')
			};
...
module.exports = Marionette.Behavior.extend({
	...
	restore: function restore(historyItem, isRedo) {
		...
		elementor.channels.data.trigger('history:after:change', history.model, history.changed, isRedo);
...
module.exports = Marionette.Behavior.extend({
	...
	add: function add(models, toView, position) {
		...
		elementor.channels.data.trigger('history:before:add', models[0]);

		toView.addChildModel(models, { at: position, silent: 0 });

		elementor.channels.data.trigger('history:after:add', models[0]);
	},

	remove: function remove(models, fromCollection) {
		elementor.channels.data.trigger('history:before:remove', models[0]);

		fromCollection.remove(models, { silent: 0 });

		elementor.channels.data.trigger('history:after:remove', models[0]);
	},
