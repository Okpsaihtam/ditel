...
class AdminFaqsPostController extends ModuleAdminController
{
  ...
  public function __construct()
  {
    ...
    $this->_join .= 'LEFT JOIN `'._DB_PREFIX_.'gomakoil_faq_shop` sa ON (a.`id_gomakoil_faq` = sa.`id_gomakoil_faq`)';
    $this->_select .= 'sa.*';
    $this->_where .= 'AND sa.id_shop IN(' . implode(',', Shop::getContextListShopID()) . ') GROUP BY sa.id_gomakoil_faq';
  ...
  protected function processBulkStatusSelection($status)
  {
    ...
    if (is_array($this->boxes) && !empty($this->boxes)) {
      foreach ($this->boxes as $id) {
        ...
        //Force to update only status property
  ...
  /**
   * Must be overrided for corrent work of multishop
   *
   * @param int $id_object
   * @return bool
   */
  ...
  public function renderForm()
  {
    ...
    $categories = faqsCategory::getCategories($id_lang,$id_shop);
    $obj = new faqsPost(Tools::getValue('id_gomakoil_faq'));
    ...
    $selected_categories = faqsPost::getAssociatedProductCategories(Tools::getValue('id_gomakoil_faq'));
    $selected = array();

    $selected_products = faqsPost::getAssociatedProducts(Tools::getValue('id_gomakoil_faq'));
    $selected_prod = array();
    ...
    $this->fields_form = array(
      ...
      'input' => array(
        ...
        array(
          ...
          'form_group_class'=> 'mpm-faqs-question-parent-category',
          'options' => array(
            'query' =>$categories,
        ...
        array(
          ...
          'form_group_class' => 'faqs-new-question-link-rewrite-form-group',
          'maxlength' => '128',
