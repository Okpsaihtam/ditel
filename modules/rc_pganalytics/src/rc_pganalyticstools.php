...
class Rc_PgAnalyticsTools
{
    ...
    // get ga categories
    public static function getGaCategories(array $products)
    {
        foreach ($products as &$product) {
    ...
    {
        $cache_key = 'Rc_PgAnalyticsTools::getGaCategory_'.$id_category;
    ...
    public static function tagProduct($product, $position, $list, $quantity_wanted)
    {
        ...
        if (isset($product['ga_variant']) && isset($product['ga_variant']['variant_price'])) {
    ...
    {
        $query = 'SELECT `product_id` AS `id_product`, `product_name`, `product_attribute_id`, `unit_price_tax_incl` 
            AS `price`, `product_quantity_refunded`
            FROM `'._DB_PREFIX_.'order_detail`
            WHERE `id_order` = '.(int)$id_order
        ;
        $products_refund = Db::getInstance()->executeS($query);
    ...
    /**
     * Prepare order model for thankyoupage
     * @param $order
     * @param $affiliation
     * @param $coupon
     * @return array
    ...
    {
        $client_id = Rc_PgAnalyticsClientId::getClientIdByCustomerId($order['id_customer']);

        // if customer client_id don't exist sen by bo cookie
        if (!$client_id) {
    ...
    {
        $client_id = Rc_PgAnalyticsClientId::getClientIdByCustomerId($id_customer);
    ...
    public static function setOrderSend($id_order, $id_shop, $sent_from = 'fo')
    {
        ...
        $order_sent_obj = new Rc_PgAnalyticsOrderSent();
    ...
    /**
     * Check if plganalytics Cookie Exist
     * @return bool
     */
    ...
    {
        $ga_cookie = new Cookie('rc_pganalytics', __PS_BASE_URI__);
    ...
    {
        $control_client = new Rc_PgAnalyticsClientId($data['id_customer']);
    ...
    public static function ajaxActionForceTransaction($params)
    {
        ...
        if (!$order_sent) {
            ...
            // Get reference url
            $ga_order['document_reference'] = self::getSourceConnection($id_order, $order_date);
            ...
            if ($products) {
                ...
                $products = self::getNamesWithoutVariant($products, $id_lang, $id_shop);
                ...
                $is_saved = self::setOrderSend($id_order, $id_shop, 'bo');

                if ($is_saved) {
                    return Rc_PgAnalyticsOrderSent::getOrderReport($id_order, $id_shop);
                }
            }
            return true;
        }
    ...
    {
        return Rc_PgAnalyticsOrderSent::removeOrder($params['id_order'], $params['id_shop']);
    }
}
